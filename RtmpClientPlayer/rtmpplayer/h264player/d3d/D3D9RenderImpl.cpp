...
{
	D3D9RenderImpl *pRender = (D3D9RenderImpl*)lpvoid;
	if( pRender != NULL )
	{
		pRender->Render( );
	}
...
HRESULT D3D9RenderImpl::Initialize(HWND hDisplayWindow, int index, void *pOwner )
{
	m_pOwner = pOwner;
	...
	*/
	...
	m_hRender = (HANDLE)_beginthreadex( NULL, 0, RenderThread, this, 0, NULL );
...
D3D9RenderImpl::~D3D9RenderImpl(void)
{
	...
	ReleaseDebug((IUnknown**)&m_pOffsceenSurface);
	ReleaseDebug((IUnknown**)&m_pTextureSurface);
	ReleaseDebug((IUnknown**)&m_pTexture);
	ReleaseDebug((IUnknown**)&m_pVertexBuffer);
	ReleaseDebug((IUnknown**)&m_pVertexShader); 
	ReleaseDebug((IUnknown**)&m_pVertexConstantTable); 
	ReleaseDebug((IUnknown**)&m_pPixelConstantTable); 
	ReleaseDebug((IUnknown**)&m_pPixelShader);
	ReleaseDebug((IUnknown**)&m_pRGBTextureSurface);
	ReleaseDebug((IUnknown**)&m_pTextTexture);

	m_overlays.RemoveAll();

	ReleaseDebug((IUnknown**)&m_pDevice);
	ReleaseDebug((IUnknown**)&m_pD3D9);
...
HRESULT D3D9RenderImpl::Display(BYTE* pYplane, BYTE* pVplane, BYTE* pUplane, int width, int height, int strideY, int strideU, int strideV)
{
	HRESULT hr = S_OK;
...
HRESULT D3D9RenderImpl::GetVideoPt( int width, int height, BYTE **dst, int &pitch )
{
	HRESULT hr = S_FALSE;
...
HRESULT D3D9RenderImpl::ReleaseVideoPt( )
{
...
HRESULT D3D9RenderImpl::ReleaseVideoPt( RECT srcRect, bool bShow )
{
	HRESULT hr = S_FALSE;
...
HRESULT D3D9RenderImpl::FillBuffer(BYTE* pY, BYTE* pV, BYTE* pU, int strideY, int strideU, int strideV)
{
	HRESULT hr = S_OK;
...
HRESULT D3D9RenderImpl::Present( RECT srcRect )
{
	HDC hdc = NULL;
	HRESULT hr = S_OK;
	RECT rect;
...
#if 0
void D3D9RenderImpl::SetTitle(TitlePos iPos, LPCSTR lpTitle, DWORD dwColor, int nLine)
{
	EnterCriticalSection( &m_section );
...
void D3D9RenderImpl::ShowCaption( bool bShow )
{
...
void
D3D9RenderImpl::SetStatus(char c, BOOL bFlag)
{
	EnterCriticalSection( &m_section );
...
void
D3D9RenderImpl::PreTextOut(HDC hdc, CString strTitle, int &iWidth, int &iHeigth)
{
	iWidth = 0;
	iHeigth = 0;
	int iStart = 0;
...
//绘制字符叠加函数
void D3D9RenderImpl::OverlayDrawText(LPCSTR pText, RECT pos, DWORD color  )
{
	HRESULT hr = S_OK;
...
void
D3D9RenderImpl::DrawTitleForCapture( HDC hdc, RECT *pRect )
{
#if 0
	do
	{
...
void D3D9RenderImpl::DrawStatusFlag( HDC hdc, RECT *rc )
{
#if 0
	if( rc == NULL )
	{
...
void
D3D9RenderImpl::SetShelter(BOOL bShelter, LPRECT lpRc, LPVOID lpReserved)
{
#if 0
	if (bShelter)
	{
...
int 
D3D9RenderImpl::SaveDC2BmpFile(const HDC hDC, const unsigned long dwWidth, const unsigned long dwHeight, 
...
BOOL D3D9RenderImpl::CaptureEx( LPCSTR lpPath, DWORD dwWidth, DWORD dwHeight )
{
	BOOL bResult = FALSE;
	...
	ReleaseDebug((IUnknown**)&pCaptureSurface);
...
DWORD D3D9RenderImpl::D3DColorToRGB( DWORD color )
{
	DWORD temp = 0;
	DWORD val  = 0;
...
#if 0
void D3D9RenderImpl::ShowIAInfo( BOOL bShow )
{
	m_bShowIAInfo = bShow;
	m_pIAFrame    = NULL;
}
...
void
D3D9RenderImpl::DrawIAInfo(CDC* pDC, IAFrame* pFrame, RECT *pRect )
{
...
BOOL
D3D9RenderImpl::IAIsAlarmEvent(IAEvent* pEvent, int nIndex)
{
	BOOL bRet = FALSE;
...
void
D3D9RenderImpl::DrawIARgn(CDC* pDC, int count, point_t* pts)
{
	int x = 0;
	int y = 0;
...
void
D3D9RenderImpl::DrawIARect(CDC* pDC, quad_t* figure)
{
...
BOOL
D3D9RenderImpl::IAFrameIsValide(IAFrame* pFrame)
{
	if (pFrame == NULL
		|| pFrame->version != 1
		|| pFrame->profile != 1
...
#if 0
double
D3D9RenderImpl::GetDistance(POINT pt1, POINT pt2)
{
...
void
D3D9RenderImpl::GetPerpendicularLine(POINT ptIn1, POINT ptIn2, POINT& ptOut1, POINT& ptOut2)
{
	if (ptIn1.x > ptIn2.x)
	{
		POINT ptTmp = ptIn1;
...
void
D3D9RenderImpl::DrawArrow(CDC* pDC, POINT ptFrom, POINT ptTo, double radian, double len)
{
...
HRESULT 
D3D9RenderImpl::FillData(BYTE* pY, BYTE* pV, BYTE* pU, int width, int height, int strideY, int strideU, int strideV )
{
	HRESULT hr = S_FALSE;
	D3DLOCKED_RECT d3drect;
	...
	do
	{
		...
		hr = m_pOffsceenSurface->LockRect(&d3drect, NULL, D3DLOCK_NOSYSLOCK|D3DLOCK_DONOTWAIT);
		if( hr != S_OK )
		{
			hr = m_pOffsceenSurface->UnlockRect();
			break;
...
int D3D9RenderImpl::GetCurMonitor( HWND hWnd )
{
	UINT iCount = 0;
	int iMon = 0;
...
void D3D9RenderImpl::ReleaseDebug( IUnknown **pObj )
{
	int ret = 0;
	if( *pObj != NULL )
	{
	...
	if( ret != 0 )
	{
		TRACE("video memory donn't release\r\n");
	}
}